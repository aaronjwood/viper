...
socket.sockets.on('connection', function(client) {
	...
	client.on('beacon', function(data) {
		...
		if(allTrackers[client.handshake.headers.origin]) {
			allTrackers[client.handshake.headers.origin].numConnections++;
			allTrackers[client.handshake.headers.origin].clients[client.id] = newUser;
		}
		
		//Otherwise create a new tracker and user and assign it to the URL
		else {
			var newTracker = new Tracker(newUser, client.handshake.headers.origin);
			allTrackers[client.handshake.headers.origin] = newTracker;
			allTrackers[client.handshake.headers.origin].numConnections = 1;
		}
	...
	client.on('disconnect', function() {
		...
		if(allTrackers[client.handshake.headers.origin]) {
			...
			var killedTracker = allTrackers[client.handshake.headers.origin].clients[client.id];
			...
			if(allTrackers[client.handshake.headers.origin].numConnections == 0) {
				delete allTrackers[client.handshake.headers.origin];
			}
			
			//Otherwise remove the specific client
			else {
				delete allTrackers[client.handshake.headers.origin].clients[client.id];
			}
